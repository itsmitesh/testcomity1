...
<#-- expected values node_type are 'categories', 'boards' -->
<#assign node_type = http.request.parameters.name.get("node_type","") />
...
<#if node_id!="" >
	<#attempt>
		<#assign solution_recent  = rest("${node_type}/id/${node_id}/solutions/recent?restapi.response_style=view&page_size=${max_solutions}&date_start=${start_date_param}&date_end=${end_date_param}").threads/>
		<#if solution_recent.thread?size == 0 >
			...
			<#if solution_recent.thread?size == 0 >
				...
				<#assign solution_recent  = rest("${node_type}/id/${node_id}/solutions/recent?restapi.response_style=view&page_size=${max_solutions}").threads/>
			</#if>
		</#if>
	<#recover>
		${.error}
...
<#else>		
		<#assign solution_recent =rest("/solutions/recent?restapi.response_style=view&page_size=${max_solutions}&date_start=${start_date_param}&date_end=${end_date_param}").threads/>
		<#if solution_recent.thread?size == 0 >
			<#-- get solutions from 365 days ago -->
			<#assign start_date_param = get_date_for_days_ago(duration2_no_of_days) />
			<#assign time_filter_used = duration2_no_of_days />
			...
			<#if solution_recent.thread?size == 0 >
				...
				<#assign time_filter_used = "none" />
				<#assign solution_recent  = rest("/solutions/recent?restapi.response_style=view&page_size=${max_solutions}").threads/>
			</#if>
		</#if>
</#if>
...
<#-- ${thread_ids} -->
<#assign views_hash =[] />
<#assign x= rest("2.0","/search?q=" + "SELECT subject, conversation.id, metrics.views FROM messages WHERE topic.id in(${thread_ids})"?url) />
<#--
